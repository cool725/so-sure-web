...
while getopts ":snpdDhcClrfg:" opt; do
  case $opt in
    ...
    p)
      ...
      ;;
    h)
      echo "Usage: $0 [-d skip db refresh] [-D debug] [-g=group test group to run] [-s populate sample policy data] [-n no network test | -p run paid test | -c run coverage | -C run coverage no network] [-l keep logs (skip force:cs check)] [-r do not flush redis after running] [filter e.g. (::Method or namespace - use \\)"
      exit 1
      ;;
  esac
done
