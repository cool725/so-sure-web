pipeline {
    ...
    stages {
        stage('Test') {
            steps {
                ...
                githubNotify account: 'so-sure', context: 'Jenkins', credentialsId: 'sosure-machine-token', description: '', gitApiUrl: '', repo: 'so-sure-web', sha: "${env.GIT_COMMIT}", status: 'PENDING', targetUrl: ''
                echo 'Running test...'
    ...
    post {
        success {
            githubNotify account: 'so-sure', context: 'Jenkins', credentialsId: 'sosure-machine-token', description: '', gitApiUrl: '', repo: 'so-sure-web', sha: "${env.GIT_COMMIT}", status: 'SUCCESS', targetUrl: ''
        }
        failure {
            githubNotify account: 'so-sure', context: 'Jenkins', credentialsId: 'sosure-machine-token', description: '', gitApiUrl: '', repo: 'so-sure-web', sha: "${env.GIT_COMMIT}", status: 'FAILURE', targetUrl: ''
        }
    }
}
