...
class PolicyTest extends \PHPUnit\Framework\TestCase
{
    ...
    /**
     * Makes sure that get next billing date works.
     */
    public function testGetNextBillingDate()
    {
        $policy = new PhonePolicy();
        $policy->setPremiumInstallments(12);
        $policy->setBilling(new \DateTime("2019-05-28 03:00"));
        $policy->setStart(new \DateTime("2019-05-31"));
        $policy->setEnd(new \DateTime("2020-05-30 23:59"));
        $policy->setStaticEnd(new \DateTime("2020-05-30 23:59"));
        $policy->setPolicyNumber("Mob/2019/5514141");
        $phone = new Phone();
        $price = new PhonePrice();
        $price->setGwp(9);
        $price->setValidFrom(new \DateTime("1999-02-12"));
        $phone->addPhonePrice($price);
        $policy->setPhone($phone);
        $policy->setPremium($price->createPremium());
        $this->addNicePayment($policy, new \DateTime("2019-05-31 15:05"))->setSuccess(true);
        $this->addNicePayment($policy, new \DateTime("2019-06-28 19:04"))->setSuccess(true);
        $this->addNicePayment($policy, new \DateTime("2019-07-28 19:03"))->setSuccess(true);
        $this->addNicePayment($policy, new \DateTime("2019-08-28 15:15"))->setSuccess(true);
        $this->addNicePayment($policy, new \DateTime("2019-09-28 15:12"))->setSuccess(true);
        $this->addNicePayment($policy, new \DateTime("2019-10-28 14:10"))->setSuccess(false);
        $policy->setStatus(Policy::STATUS_UNPAID);
        // boogaloo
        $finalDate = new \DateTime("2019-10-31 00:00", SoSure::getSoSureTimezone());
        for ($i = 0; $i < 432000; $i++) {
            $finalDate->add(new \DateInterval("PT1S"));
            $this->assertEquals(
                new \DateTime("2019-11-27"),
                $policy->getPolicyExpirationDate($finalDate)
            );
            $this->assertFalse($policy->shouldCancelPolicy("Mob", $finalDate));
        }
    }

    public function addNicePayment($policy, $date)
    {
